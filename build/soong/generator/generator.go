...
func init() {
	android.RegisterModuleType("swift_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = swiftExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = swiftExpandVariables(ctx, dep_file)
	...
	exCmd := swiftExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestName := "swift.sbox.textproto"
	desc := "generate"
	name := "generator"
