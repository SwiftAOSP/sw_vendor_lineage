...
function setup_vendor() {
    ...
    if [ ! -d "$SWIFT_ROOT" ]; then
        echo "\$SWIFT_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$SWIFT_ROOT/$OUTDIR" ]; then
        mkdir -p "$SWIFT_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$SWIFT_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$SWIFT_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$SWIFT_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$SWIFT_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$SWIFT_ROOT"/prebuilts/tools-swift/common/smali/baksmali.jar
        export SMALIJAR="$SWIFT_ROOT"/prebuilts/tools-swift/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$SWIFT_ROOT"/prebuilts/tools-swift/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$SWIFT_ROOT"/prebuilts/tools-swift/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$SWIFT_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$SWIFT_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$SWIFT_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$SWIFT_ROOT"/vendor/swift/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$SWIFT_ROOT"/"$OUTDIR"/radio
